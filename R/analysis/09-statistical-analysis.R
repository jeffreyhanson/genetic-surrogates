...
multi.spp.with.cost.SDF <- multi.spp.with.cost.DF %>%
	...
	mutate(Metric=revalue(Metric, c('adaptive.held'='Adaptive variation', 
		'neutral.held'='Neutral variation'))) %>%
	mutate(Prioritisation.Metric=interaction(Prioritisation,Metric)) 
multi.spp.with.cost.SDF$Prioritisation <- factor(multi.spp.with.cost.SDF$Prioritisation, levels=c('Amount','Surrogate','Genetic'))

# model
multi.spp.with.cost.GLMM <- suppressWarnings(glmer(value ~ Prioritisation * Metric + (1 | Species), family='binomial', data=multi.spp.with.cost.SDF, nAGQ=20))
multi.spp.with.cost.GLMM1 <- suppressWarnings(glmer(value ~ Prioritisation + Metric + (1 | Species), family='binomial', data=multi.spp.with.cost.SDF, nAGQ=20))
multi.spp.with.cost.GLMM2 <- suppressWarnings(glmer(value ~ Prioritisation + (1 | Species), family='binomial', data=multi.spp.with.cost.SDF, nAGQ=20))
multi.spp.with.cost.GLMM3 <- suppressWarnings(glmer(value ~ 1 + (1 | Species), family='binomial', data=multi.spp.with.cost.SDF, nAGQ=20))

multi.spp.with.cost.AOV <- suppressWarnings(anova(multi.spp.with.cost.GLMM, test="LRT"))
multi.spp.with.cost.AOV1 <- suppressWarnings(anova(multi.spp.with.cost.GLMM, multi.spp.with.cost.GLMM1, test="LRT"))
multi.spp.with.cost.AOV2 <- suppressWarnings(anova(multi.spp.with.cost.GLMM1, multi.spp.with.cost.GLMM2, test="LRT"))
multi.spp.with.cost.AOV3 <- suppressWarnings(anova(multi.spp.with.cost.GLMM2, multi.spp.with.cost.GLMM3, test="LRT"))

# post-hoc
multi.spp.with.cost.GLMM2 <- suppressWarnings(glmer(value ~ Prioritisation.Metric + (1 | Species), family='binomial', data=multi.spp.with.cost.SDF, nAGQ=20))
multi.spp.with.cost.MCP <- summary(
	glht(multi.spp.with.cost.GLMM2,
		linfct=mcp(Prioritisation.Metric='Tukey')),
	adjusted('bonferroni'))
