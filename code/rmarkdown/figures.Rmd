...
suppressWarnings(
	ggplot(aes(x=surrogate.held, y=genetic.held), data=curr.DF) +
		theme_classic() +
		theme(strip.background=element_rect(fill='grey20'), strip.text=element_text(color='white', face='italic', size=10),
		...
		geom_text(aes(label=R2_repr), data=curr.DF2, parse=TRUE, hjust=0, size=3, color='blue', alpha=0.8) +
		facet_wrap(~ Species, ncol=4)
)
```

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=calc.fig.height2(n_distinct(geo.surrogacy.DF$Species)), fig.cap='The relationship between the proportion of neutral genetic variation secured in a prioritizaiton and the proportion of geographic variation it also secures. Conventions are detailed in Figure SXXX.'}
# format data
...
suppressWarnings(
	ggplot(aes(x=surrogate.held, y=genetic.held), data=curr.DF) +
		...
		suppressWarnings(stat_smooth(method='glm', method.args=list(family='binomial'), fullrange=TRUE,
			se=FALSE, fill='blue', alpha=0.8)) +
		geom_text(aes(label=R2_repr), data=curr.DF2, parse=TRUE, hjust=0, size=4, color='blue', alpha=0.8) +
		facet_wrap(~ Species, ncol=4)
)
```
