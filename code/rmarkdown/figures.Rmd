...
panel.DF <- grid.sub.FPLY %>% 
	summarize(lat=min(lat)+(diff(range(lat))*0.98),
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.height=4.5, fig.cap='The relationship between surrogates and genetic variation secured in prioritizations. Lines indicate the average trend for each species. Grey sillhouettes indicate their uncertainty in these trends. Histogram inserts show the distribution of $R^2$ values. Panel (a) shows the relationship between the proportion of environmental variation and adaptive genetic variation secured in a prioritization. Similarly, panel (b) shows that for geographic surrogate targets and neutral genetic variation.'}
# make figure
...
p1.hist <- ggplot(aes(R2),
		data=filter(surrogacy.model.summary.DF, Type=='Environmental')) +
	geom_histogram(fill='grey10') +
	ylab('\nFrequency') + xlab(expression(atop('Effectiveness of environmental', variables~(R^2~coefficient)))) +
	theme_classic() + theme(axis.line.x=element_line(color='black'),
		axis.line.y=element_line(color='black'),
		plot.background=element_rect(fill='transparent', colour='transparent')) +
		geom_text(x=-1.375, y=0.95, label='(c)') +
		coord_cartesian(xlim=c(-1.5, 1))

p2.hist <- ggplot(aes(R2),
		data=filter(surrogacy.model.summary.DF, Type=='Geographic')) +
	geom_histogram(fill='grey10') +
	ylab('\nFrequency') + xlab(expression(atop('Effectiveness of geographic', variables~(R^2~coefficient)))) +
	theme_classic() + theme(axis.line.x=element_line(color='black'),
		axis.line.y=element_line(color='black'), 
		plot.background=element_rect(fill='transparent')) +
		geom_text(x=-1.375, y=0.95, label='(d)') +
		coord_cartesian(xlim=c(-1.5, 1))
