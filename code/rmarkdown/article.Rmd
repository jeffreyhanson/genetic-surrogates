...
To describe the environmental characteristics of each planning unit, we obtained data for 19 bioclimatic variables across the extent of the study area [at $30^{\prime\prime}$ resolution; @r33]. We used this data set because it is commonly to map habitat suitability in conservation planning [@r493]. These layers and the planning units were projected into an equal-area coordinate system (Europe Lambert Conformal Conic; $\texttt{ESRI:102014}$). The average value in each planning unit for each of the 19 layers was calculated. To reduce dimensionality, for each species, the climatic values associated with the planning units occupied by the species were subject to principal components analysis (PCA; Table S1). On average, the first `r as.character(as.english(surrogate.params.LST[[MODE]]$number.components))` principal components (PCs) cumulatively explained `r round(mean(spp.pca.DF[[2]]*100),2)` % $\pm$ `r round(sd(spp.pca.DF[[2]]*100),2)` of the variation. We then used the first `r as.character(as.english(surrogate.params.LST[[MODE]]$number.components))` principal components to characterise climatic variation among the planning units for each species (Figure S2). Thus we constructed a `r as.character(as.english(surrogate.params.LST[[MODE]]$number.components))`-dimensional environmental space for each species--wherein each planning unit occupies a single point--as a potential surrogate for adaptive variation for that species. We expect planning units further apart from each other in this environmental space to contain individuals with increasingly different patterns in their adaptive loci.
