...
<!-- surrogacy analysis -->
We found the environmental and geographic variables to be moderately effective surrogates for genetic variation in most species (Figure 1). For each species, we randomly generated prioritisations, and calculated the proprotion of environmental, geographic, adaptive (if present), and neutral genetic variation they sampled. We found the relationship between the proportion of adaptive genetic variation and environmental variation secured in a given prioritisation to depend on the species for which the prioritisation was generated (species $\times$ proportion of environmental variation interaction term; ${\chi^2}_{`r env.surrogacy.AOV$df[[4]]`} = {`r round(env.surrogacy.AOV$Resid..Dev[[4]],2)`}; P `r pvalString(env.surrogacy.AOV$Pr..Chi.[[4]])`$). Similarly, we found the relationship between the proportion of neutral genetic variation and geographic variation secured in a prioritisation to depend on the species for which the prioritisation was generated (${\chi^2}_{`r round(geo.surrogacy.AOV$df[[4]],2)`} = `r round(geo.surrogacy.AOV$Resid..Dev[[4]],2)`$; $P$ `r pvalString(geo.surrogacy.AOV$Pr..Chi.[[4]])`). For all species, we found these relationship to be significantly different to zero (environmental: minimum \textit{z} = `r round(max(env.surrogate.spp.DF$statistic),2)`, maximum $P `r pvalString(max(env.surrogate.spp.DF$p.value))`$; geographic: minimum \textit{z} = `r round(min(geo.surrogate.spp.DF$statistic),2)`, maximum $P `r pvalString(max(env.surrogate.spp.DF$p.value))`$; Table S3). After establishing that the proportion of surrogate and genetic variation were correlated in randomly generated prioritisations, our final step was to determine if prioritisations were more effective when generated with additional surrogate-based targets.
...
We chose this data set for several reasons. First, to address our aims, we required genetic data for all species to be collected using a standardised sampling scheme, and this data set used a grid to collect samples. Second, because conservation planning exercises typically consider tens to hundreds of species with different evolutionary histories and life histories [eg. @rXXX], we needed a data set that contains a comparable set of species so that our results will be relevant to planners. Third, we required the study area to contain broad-scale environmental variation to test the effectiveness of environmental variables, and this data set spans across large attitudinal gradients. Although this data set contains AFLPs which not ideal as genomic representatives (cf. single nucleotide polymorphisms; SNPs), no multi-species genomic data set with similar properties currently exists. Thus this data set was well suited for our study.

This data set has previously been used to address a range of different questions. Many studies have used this data set to explore patterns of adaptive [@r484; @r451; @r475; @r455; @r485] and neutral genetic variation [@r478; @r451; @r486]. However, to our knowledge, only a single study has used this data set to explore the effectiveness of a surrogate in the context of conservation planning. Taberlet _et al._ [-@r486] investigated whether prioritisations generated using the spatial distribution of a comprehensive set species (> 300) could adequately secure the intra-specific genetic variation for 27 species. They found that the prioritisation based on species distributions was substantially different to the prioritisation based on genomic data. Their results suggest that prioritisations based on species distribution data alone will be unable to secure intra-specific genetic variation. However, whether or not environmental and geographic data can result in more prioritisations remains untested.
