...
plot.spp.spaces <- function(i) {
  ...
  curr.FPLY <- grid.FPLY[,
    c(adapt.col, neutral.col, env.col,
    'long','lat','group'),drop=FALSE]
  curr.FPLY <- curr.FPLY[which(!is.na(curr.FPLY[[1]])),]
  curr.FPLY <- curr.FPLY %>% gather(d, value, contains('adaptive'), contains('neutral'), contains('env')) %>%
    mutate(
      d=gsub(paste0(unique(spp.samples.DF$species)[i], '_'), '', d, fixed=TRUE),
      d=gsub('_', ' ', d, fixed=TRUE))
  for (j in seq_len(length(adapt.col)+length(neutral.col)+length(env.col)))
...
calc.fig.height <- function(i) {
  ...
  env.col <- grepl(paste0(unique(spp.samples.DF$species)[i], '_env_d'), names(grid.FPLY), value=TRUE, fixed=TRUE) %>%
    sum()
