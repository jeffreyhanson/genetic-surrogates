...
suppressWarnings({
	ggplot(data=bic.DF) +
		...
		theme(
			strip.background = element_rect(fill='grey20'),
			strip.text = element_text(color='white', face='italic', size=1)
		) +
	...
	function(i) {
		if (!is.null(spp.BayeScan.sample.loci.subset.LST[[i]])) {
			data.frame(
				...
				Bayescan_qval=spp.BayeScan.LST[[i]]$bayescan@results@summary[[4]],
				...
				PCadapt_chi2_statistic=spp.pcadapt.LST[[i]]$chi2.stat,
...
# save to file
write.table(loci_classification.DF, 'article/Table_S2', sep=',', row.names=FALSE)
```
