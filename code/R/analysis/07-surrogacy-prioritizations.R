...
clusterExport(clust, c('spp.samples.sub.DF','grid.sub.DF','rapr.params.LST', 'MODE', 'species.prioritisation', 'ru'))
...
env.surrogacy.prioritisations <- llply(
	...
	function(i) {
		...
		curr.random.n <- sample.int(length(curr.pu), rapr.params.LST[[MODE]]$surrogacy.analysis$random.replicates, replace=TRUE)
		...
		curr.sol.MTX<-matrix(0, ncol=nrow(grid.sub.DF), nrow=rapr.params.LST[[MODE]]$surrogacy.analysis$random.replicates)
