...
```{r, echo=FALSE, fig.width=15, fig.height=14, fig.cap='Distribution of populations. Each panel depicts a different species. Squares denote planning units. Pie charts denote the location of sampled individuals, and colors denote the probability that they belong to different populations. Note that pie charts have been jittered to make individuals in close proximity visible.'}
# prepare data
pop.probs.DF <- llply(
	seq_along(unique(spp.samples.DF$species)),
	function(i) {
...
# create grobs
grobLST <- llply(
	seq_along(unique(pop.probs.DF$species)),
	function(i) {
		# init
		...
		print(curr.GROB, vp=vplayout(1:100,1:100))
		# add in pies
		for (j in unique(curr.spp.DF$cell)) {
			...
			curr.lat <- (curr.cell.DF$grid.latitude[1]-bbox['ll.lat'])/abs(bbox['ur.lat']-bbox['ll.lat'])
			...
			# render pie
			print(curr.pie, vp=viewport(x=curr.lon, y=curr.lat, width=0.05, height=0.05))
		}
	}
)
```

```{r, echo=FALSE, fig.width=15, fig.height=14, fig.cap='Population membership probabilities. Each panel depicts a different species. Each vertical bar represents a different individual, and colors show the probability that it belongs to different populations. This type of plot is also known as a \texttt{Structure} plot.'}
# prepare data
pop.probs.DF <- ldply(
	seq_along(spp.StructureData.LST),
	function(i) {
		spp.Mclust.LST[[i]]$mclust$z
			as.data.frame() %>%
			mutate(Species=unique(spp.samples.DF$species)[i]) %>%
			gather(Population,Probability,-Species) %>%
			mutate(Population=as.numeric(gsub('V', '', Population, fixed=TRUE)),
...
ggplot(
	...
	xlab('') +
	ylab('Probability of population membership (%)') +
	facet_wrap(~ Species)
```
