...
```{r, echo=FALSE, fig.width=9.5, fig.height=10, out.width='7.5in', out.height='8.5in', fig.cap=paste0('Species distributions. Squares represent planning units. For a given species, planning units that were found to be inhabited are denoted with bright blue. Note that \textit{',missing.species,'} was omitted from analysis.')}
...
plot.spp.nmds <- function(i) {
	...
	adapt.col <- grep(
		...
		names(grid.sub.FPLY), value=TRUE, fixed=TRUE
	)
	neutral.col <- grep(
	...
	if (length(c(adapt.col, neutral.col)==0)
		return(NULL)
	curr.FPLY <- grid.FPLY[,
	...
	curr.FPLY <- curr.FPLY %>% gather(d, value, contains('adaptive'), contains('neutral')) %>% mutate(
		d=gsub(paste0(unique(spp.samples.sub.DF$species)[i], '_'), '', d, fixed=TRUE),
...
calc.fig.height <- function(i) {
	# init
	adapt.col <- paste0(unique(spp.samples.sub.DF$species)[i], '_adaptive_d1')
	neutral.col <- paste0(unique(spp.samples.sub.DF$species)[i], '_neutral_d1')
	n.plots <- 0 
	# count adaptive plot
