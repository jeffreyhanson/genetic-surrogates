...
p1 <- ggplot(mapping=aes(x=factor(Surrogate.target),y=genetic.held, color=Method)) +
	...
	geom_line(data=data.frame(summarize(group_by(env.correlation.DF, Surrogate.target, Method), genetic.held=median(genetic.held)))) +
...
p2 <- ggplot2(mapping=aes(x=factor(Surrogate.target),y=genetic.held, color=Method)) +
	...
	geom_line(data=data.frame(summarize(group_by(geo.correlation.DF, Surrogate.target, Method), genetic.held=median(genetic.held)))) +
	theme_classic() +
	scale_color_manual(name='Prioritization', values=c('Optimal'='black', 'Random'='grey50'))
