...
correlation.PDF2 <- correlation.PDF %>%
	group_by(Type, Surrogate.target, Method) %>% 
	summarize(genetic.held=max(genetic.held, na.rm=TRUE)+0.1) %>%
	ungroup() %>%
	mutate(Combination=as.character(paste0(Type,'.',Surrogate.target,'.',Method))) %>% 
	left_join(
		data.frame(
			letter=unname(toupper(cld(correlation.GLHT)$mcletters$Letters),
			Combination=as.character(names(toupper(cld(correlation.GLHT)$mcletters$Letters))),
...
# make figure
p1 <- ggplot(aes(x=Surrogate.target,y=genetic.held),
		data=filter(correlation.PDF, Type=='Environmental')) +
	geom_boxplot(position=position_dodge(0.9)) +
	xlab('Environmental surrogate target (%)') + 
	ylab('Adaptive genetic\nvariation secured (%)') +
	geom_abline(intercept=rapr.params.LST[[MODE]]$surrogacy.analysis$genetic.target,
		slope=0, color='red', linetype='dashed') +
	geom_text(aes(x=0.55, y=1.115, label='(a)'), color='black', size=5) +
	theme_classic() +
	theme(legend.position='none') +
	geom_text(aes(x=Surrogate.target, y=genetic.held, label=letter),
		data=filter(correlation.PDF2, Type=='Environmental'),
		position=position_dodge(0.9))

p2 <- ggplot(aes(x=Surrogate.target,y=genetic.held),
	...
	theme(legend.position='none') +
	geom_text(aes(x=Surrogate.target, y=genetic.held, label=letter),
		data=filter(correlation.PDF2, Type=='Geographic'),
		position=position_dodge(0.9))
...
scenario.PDF <- scenario.DF %>% group_by(Prioritisation, Context, Metric) %>% 
	...
	left_join(
		data.frame(
			letter=unname(toupper(cld(scenario.GLHT)$mcletters$Letters)),
			Combination=as.character(names(toupper(cld(scenario.GLHT)$mcletters$Letters)))
