...
env.correlation.DF2 <- env.correlation.DF %>%
	...
	summarize(genetic.held=max(genetic.held)+0.1) %>%
...
p1 <- ggplot(aes(x=Surrogate.target,y=genetic.held, color=Method),
	...
	geom_text(aes(x=0.75, y=1, label='(a)'), color='black', size=5) +
	theme_classic() +
...
p2 <- ggplot(aes(x=Surrogate.target,y=genetic.held, color=Method),
	...
	geom_text(aes(x=0.75, y=1, label='(b)'), color='black', size=5) +
	theme_classic() +
...
suppressWarnings({
	ggplot(aes(x=Prioritisation,y=value),
		...
		geom_text(aes(x=0.7, y=1, label=panel.letter),
			data=results.PDF) +
