...
correlation.PDF <- correlation.sub.DF %>%
	group_by(Surrogate.target, Type, Species) %>% 
	summarize(genetic.held=mean(genetic.held, na.rm=TRUE)) %>%
	data.frame()

correlation.PDF2 <- correlation.PDF %>%
	group_by(Type, Surrogate.target, Type) %>% 
	summarize(genetic.held=max(genetic.held, na.rm=TRUE)+0.1) %>%
	ungroup() %>%
	mutate(Combination=as.character(paste0(Surrogate.target,'.',Type))) %>% 
	left_join(
		data.frame(
			letter=unname(correlation.fisher.cld$Letters),
			Combination=names(correlation.fisher.cld$Letters),
...
grid.FPLY <- grid.FPLY %>% gather(
		...
		Prioritization=revalue(Prioritization, c('Amount'='Amount\ntargets', 'Surrogate'='Amount & surrogate\ntargets', 'Genetic'='Amount & genetic\ntargets')),
		Prioritization=factor(as.character(Prioritization), levels=c('Amount\ntargets', 'Amount & surrogate\ntargets', 'Amount & genetic\ntargets')),
		Context=factor(
			gsub(' (', '\n(', Context, fixed=TRUE),
...
scenario.PDF <- scenario.DF %>% group_by(Prioritisation, Context, Metric) %>% 
	...
	left_join(
		data.frame(
			letter=unname(scenario.fisher.cld$Letters),
			Combination=names(scenario.fisher.cld$Letters)
...
suppressWarnings({
	ggplot(aes(x=Prioritisation,y=value),
		...
		geom_text(aes(x=0.5, y=1.075, label=panel.letter),
			data=scenario.PDF) +
