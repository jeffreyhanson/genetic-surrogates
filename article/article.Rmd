...
First, we grouped individuals into genetic lineages for each species. The purpose of this step was to avoid misclassifying loci as adaptive. We used Gaussian mixture models to assign individuals to populations [implemented in $\texttt{mclust}$ R package; @r477; @r467]. Briefly, for each species, we fit a suite of models to estimate the probability that individuals belonged different populations (using default priors and covariance structures). For each parametrization, we fit a set of models assuming varying numbers of total populations (\textit{G} = 1--`max(r mclust.params.LST[[MODE]]$G)`). We identified the most parsimonious models (lowest Bayesian Information Criterion $BIC$ statistic; Figures S4), and used these to group individuals into populations (Figure S5).

We identified loci under selection. For each species, we ran $\texttt{Bayescan}$ [version 2.1; @r456] using the population memberships identified in the previous analysis (1:1 prior odds; `r bayescan.params.LST[[MODE]]$nbp` pilot runs; `r bayescan.params.LST[[MODE]]$burn` burn-in iterations; `r bayescan.params.LST[[MODE]]$n` post-burn-in iterations thinned by `r bayescan.params.LST[[MODE]]$thin` iterations; Table S2). To avoid false-positive, we omitted loci where the global frequency of the minor allele was $\geq 0.05$ (following guidelines in the $\texttt{Bayescan}$ manual). Additionally, to prevent individuals with unclear ancestry from biasing this analysis, we omitted individuals if there was considerable uncertainty in their population membership (maximum probability $\leq `r mclust.params.LST[[MODE]]$probability.threshold`$ % ). We ran `r bayescan.params.LST[[MODE]]$reps` replicates per species using a suitable false discovery rate to ensure convergence [FDR $\leq `r bayescan.params.LST[[MODE]]$fdr`$; @r466]. 
